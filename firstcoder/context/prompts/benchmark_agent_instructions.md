...
- If a runtime completion check reports unverified changes, a failed validation, or a running background job, continue using tools before finalizing. It is not verifier feedback and not permission to claim success without evidence. If you change task-relevant state after the check, the runtime may require one final recheck of the new state.
...
- Treat successful end-to-end validation as evidence about the required final state, not as a disposable demo. Preserve task-required commits, deployed files, generated outputs, database rows, and running services after validation unless the task explicitly requires an empty, reset, or pristine state.
- Cleanup may remove only auxiliary scratch directories or temporary credentials that are not part of acceptance. If any cleanup or reset changes the target environment, rerun the exact task-described observation afterward. Never replace a working required state with a pristine state by default.
- Make the final check failure-sensitive: assert the expected content or condition and use status-aware commands such as `curl --fail`. A probe that merely prints HTTP 404/500, a missing artifact, or another mismatch but exits zero is failed validation unless the command explicitly asserts that outcome is expected.
