...
每个 B 轮默认最多执行 6 个工具回合、12 次 provider 调用、240 秒循环时间，主模型输出上限为 4096 tokens；对 OpenAI SDK 路由还会注入仅本次 trial 生效的 45 秒 HTTP 请求超时（严格小于 240 秒轮次上限，为失败分类重试保留时间）。可通过 runner 的对应 `--max-*` 与 `--provider-timeout-seconds` 参数覆盖，但 HTTP 请求超时必须严格小于轮次上限，且三臂必须使用相同值。
